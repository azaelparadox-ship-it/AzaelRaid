...
function buildVoteEmbed(raid) {
  ...
  const bench = Object.values(votes).filter(v => v.role === "Bench");
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `🪑 Bench (${bench.length})`,  value: fmtVoters(bench), inline: false }
    )
...
function buildRegistrationEmbed(raid) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `🪑 Bench (${bench.length}) — Renfort non prioritaire`, value: fmt(bench), inline: false }
    )
